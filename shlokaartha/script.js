...
let MEANINGS_FULL_ENG = {};
let MEANINGS_FULL_HIN = {};
let MEANINGS_WORD_ENG = {};
let MEANINGS_WORD_HIN = {};

async function loadMeanings() {
    const [fullEng, fullHin, wordEng, wordHin] = await Promise.all([
        fetch("../meanings_full_eng.json").then(r => r.json()),
        fetch("../meanings_full_hin.json").then(r => r.json()),
        fetch("../meanings_wordwise_eng.json").then(r => r.json()),
        fetch("../meanings_wordwise_hin.json").then(r => r.json())
    ]);

    MEANINGS_FULL_ENG = fullEng;
    MEANINGS_FULL_HIN = fullHin;
    MEANINGS_WORD_ENG = wordEng;
    MEANINGS_WORD_HIN = wordHin;
}
...
const meaningLanguageSelect = document.getElementById("meaningLanguageSelect");
...
// ---------------------------------------------------------
// 🔐 LOCAL STORAGE KEYS
const STORAGE_KEY = "gitaAppState_shlokaartha";
...
function loadState() {
    ...
    try {
        ...
        meaningLanguage = state.meaningLanguage || "ENG";
        meaningLanguageSelect.value = meaningLanguage;
...
function renderShlokaView() {
    ...
    currentDisplayVerses.forEach(verseKey => {
        ...
        // Sanskrit text
        const shlokaText = document.createElement('p');
        shlokaText.className = "mb-3 text-gray-800 whitespace-pre-line";
        shlokaText.textContent = text || "Shloka text not available.";
        ...
        verseBlock.appendChild(shlokaText);
        verseBlock.appendChild(wordDiv);
        verseBlock.appendChild(fullDiv);
...
//Hooking up Language Selector
meaningLanguageSelect.addEventListener("change", (e) => {
    meaningLanguage = e.target.value;
    renderShlokaView();   // refresh meanings
    saveState();
});
